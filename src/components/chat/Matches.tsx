              ...
              >
                ...
                <div className="flex-1 text-left relative">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[8px] uppercase tracking-widest text-gray-600 font-bold shrink-0">
                       {m.updatedAt ? new Date(m.updatedAt.toString()).toLocaleDateString() : 'Now'}
                    </span>
                  </div>
                  <p className={`text-xs truncate italic ${m.lastMessageSenderId !== profile.uid && m.lastMessageSenderId && !m.isRead ? 'text-white font-medium' : 'text-gray-500'}`}>
                ...
                <ArrowRight size={16} className="text-gray-700 group-hover:text-[#F27D26] group-hover:translate-x-1 transition-all opacity-0 group-hover:opacity-100" />
              </motion.button>
            ))}
          </AnimatePresence>
        </div>
